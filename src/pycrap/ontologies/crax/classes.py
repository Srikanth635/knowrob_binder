...
class PhysicalAttribute(Base):
    ...
    """
#
class ForceAttribute(Base): # PhysicalAttribute
    """
    The value of a force dynamical characteristic. An example is the force exerted on another object when pushing it.
    """
#
class FrictionAttribute(Base): # PhysicalAttribute
    """
    The resistance that one surface or object encounters when moving over another.
    """
#
class NetForce(Base): # ForceAttribute
    """
    The accumulated force acting upon the object.
    """
#
class StaticFrictionAttribute(Base): # FrictionAttribute
    """
    Friction between two touching objects that do not move relative to each other.
    """
#
class KineticFrictionAttribute(Base): # FrictionAttribute
    """
    Friction that occurs when two touching objects are moving relative to each other.
    """

# New classes under ForceAttribute
class ForceMinimalExertion(Base): # ForceAttribute
    """
    Minimal force exerted during free-space movement, e.g., moving a knife through the air before contact.
    """

class ForceProprioceptionFeedback(Base): # ForceAttribute
    """
    Force sensed at joints or end-effectors, providing feedback during movement or grasping.
    """

class ForceMinorResistance(Base): # ForceAttribute
    """
    Small opposing force encountered, such as air resistance or workspace constraints, during movement.
    """

class ForceContactForce(Base): # ForceAttribute
    """
    Initial force applied when grasping an object, marking the onset of contact.
    """

class ForceResistanceFromObject(Base): # ForceAttribute
    """
    Force exerted back by an object due to its shape, affecting grasp efficiency.
    """

class ForceEquilibriumHold(Base): # ForceAttribute
    """
    Stable force maintained to hold an object securely during grasping.
    """

class ForceEquilibriumState(Base): # ForceAttribute
    """
    Minimal force state maintained before contact with an object.
    """

class ForceGravityCompensation(Base): # ForceAttribute
    """
    Force applied to hold an object, like a knife, stable against the effect of gravity.
    """

class ForcePreloading(Base): # ForceAttribute
    """
    Slight downward force applied before initiating a cutting action.
    """

class ForceShearForce(Base): # ForceAttribute
    """
    Lateral force exerted by a blade during cutting, causing shear stress on the object.
    """

class ForceCompressiveForce(Base): # ForceAttribute
    """
    Downward pressure exerted on an object, such as an apple, during cutting.
    """

class ForceBlockageEvent(Base): # ForceAttribute
    """
    Initial resistance force encountered before a blade penetrates an object.
    """

class ForceBreakageEvent(Base): # ForceAttribute
    """
    Force at which an object, like an apple, fractures as cutting completes.
    """

class ForceModulationDuringCutting(Base): # ForceAttribute
    """
    Dynamic adjustment of pressure applied during the cutting process.
    """

class ForceLoadReduction(Base): # ForceAttribute
    """
    Gradual reduction in force applied after completing a cutting action.
    """

class ForceEquilibriumReset(Base): # ForceAttribute
    """
    Force state where the arm stabilizes after completing a task.
    """

# New classes under FrictionAttribute
class ForceFrictionGrip(Base): # StaticFrictionAttribute
    """
    Static friction force that prevents slipping during grasping of an object.
    """

class ForceFrictionResistance(Base): # KineticFrictionAttribute
    """
    Kinetic friction force encountered by a blade from an object's fibers during cutting.
    """

# New class under NetForce
class ForceSeparationEvent(Base): # NetForce
    """
    Accumulated force causing apple halves to move apart under gravity after cutting.
    """
