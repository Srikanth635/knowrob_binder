...
class exerts_force(BaseProperty):
	"""
	A relation indicating that a physical object exerts a specific force attribute, e.g., 'a knife exerts shear force on an apple'.
	"""


class is_exerted_by(BaseProperty):
	"""
	A relation indicating that a force attribute is exerted by a physical object, e.g., 'shear force is exerted by a knife'.
	"""


class resists_force(BaseProperty):
	"""
	A relation indicating that a physical object resists a specific force attribute, e.g., 'an apple resists friction force from a knife'.
	"""


class is_resisted_by(BaseProperty):
	"""
	A relation indicating that a force attribute is resisted by a physical object, e.g., 'friction force is resisted by an apple'.
	"""


class occurs_during(BaseProperty):
	"""
	A relation indicating that a force attribute occurs during a specific action, e.g., 'compressive force occurs during cutting'.
	"""


class applied_to(BaseProperty):
	"""
	A relation indicating that a force attribute is applied to a physical object, e.g., 'compressive force is applied to an apple'.
	"""
